# -*- coding: utf-8 -*-
"""雀魂段位戦ポイント推移グラフ作成ツール のコピー

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/17s4D8oTX8uaArT2ftu5Om7lIEN8qJ9iD

# **雀魂段位戦ポイント推移グラフ作成ツール**
## **利用手順**
1. お手持ちのGoogleアカウントでログインしている必要があります。
また、Androidスマホの場合は、Chromeのメニューから、「PC版サイト」を選択しておく必要があるようです。
1. 下部のフォーム「プレイヤー名：」に対象とするプレイヤー名を入力し、フォーム左部の再生ボタン(▷)を押してください。
1. 「警告: このノートブックは Google が作成したものではありません。」というメッセージが出たら、右下の「このまま実行」を押してください。
1. しばらく待つとページ最下部にグラフが表示されますので、右クリックやドラッグ・アンド・ドロップで保存してご使用ください。

黄色は金の間、緑は玉の間、赤は王座の間の対局を表し、水平線は各段位における原点と昇段ラインを表しています。

## **注意**
- 利用者が自身の段位戦履歴を振り返る用途を想定しています。他のプレイヤーのグラフを無断で公開することはトラブルの元となる可能性がありますのでご注意ください。
- 詳細な動作確認はしていません。ご利用は自己責任でお願いいたします。本プログラムは予告なく削除・変更する可能性があります。
- 本プログラムは雀魂牌譜屋へのアクセスを含んでいるため、サーバー負荷に注意して常識的な範囲でご利用ください。
- 魂天のポイントは[0.0, 20.0]の範囲ですが、見やすさのため、グラフ上では500倍の[0, 10000]の範囲に引き伸ばしています。
- 旧王座での対戦があると表示が変になるかもしれません。
- 金の間以上のデータしか取得できないため、雀傑で銀の間で打ったことがあるとポイントがずれるかもしれません。

Produced by IrisCat ( https://twitter.com/iriscatx ), Thanks to 雀魂( https://mahjongsoul.com ) & 雀魂牌譜屋( https://saki.sapk.ch/ )
"""
